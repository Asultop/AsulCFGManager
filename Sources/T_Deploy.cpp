...
{
    initWidget("部署 CFG","安装","选择符合格式的安装包\n我们会为您解析数据并自动安装到您的CFG目录下");

    // waitingDialog->show();
    emit pushTextToWaitingDialog("寻找文件目录");
    if(tempDir!=nullptr)
        gLocation=tempDir;
    else
        gLocation=new QTemporaryDir();
    this->setAcceptDrops(true);
    emit pushTextToWaitingDialog("获取目录");
    // getAllPath();
    qDebug()<<"\n=====getAllPath()======\nSteamPath:"
             <<steamPath
             <<"\nCFGPath:"<<CFGPath
    ...
    connect(scanFileButton,&ElaPushButton::clicked,[=](){
        scanFileButton->setText("扫描中..");
        QStringList RET=scanExistProject();
        qDebug()<<"RET:"<<RET;
        //QTimer::singleShot(100,this,[=](){

        scanFileButton->setText("扫描CFG文件夹");
        ...
        if(!RET.isEmpty()){
            ...
            QString buffer=QString("存在配置:[%1]").arg(nameDir.join(","));
            parentWindow->succTips("扫描",buffer);
            ElaMessageBar::success(ElaMessageBarType::TopLeft,"扫描",buffer,2000);
            qDebug()<<"Succ:"<<QString("存在配置:[%1]").arg(nameDir.join(","));
        }else parentWindow->normTips("扫描","未找到存在的配置");
    });
    ...
    this->addTopWidget(GenerateArea(this,new ElaText("功能",this),selectFileLayout));
...
bool T_Deploy::UnCompressFiles2(const QString &archivePath, const QString &extractDir){
    ...
    qDebug()<<"processArgu"<<process.arguments();
    process.start();
...
QStringList T_Deploy::scanExistProject(){
    emit pushTextToWaitingDialog("寻找本地CFG管理文件");
...
void T_Deploy::generateScrollPageLayout(QString CFGFileLocation, ElaFlowLayout *cVLayout) {
    ...
    qDebug()<<"getIn generateScrollPageLayout";
    bool showable=true;
    ...
    if(ExistDir.contains(CFGFileLocation)){
        CFGBreifInfo->setCardButtontext("管理");
        CFGBreifInfo->setInteractiveTips("已安装");
    }
    ...
    foreach (QString asulFile, fileNames) {
        ...
        for (int i = 0; i < lines.size(); ++i) {
            ...
            if(line.startsWith("key") || line.startsWith("must")){
                ...
                int colNum=sumContent->addLabel(detailText);
                sumAsul->addLabel(detailText);
                qDebug()<<"InLine"<<QString("%WAITING_REPLACE%").replace("%WAITING_REPLACE%","\%1");
                detailText->setProperty("num",colNum);
        ...
        ElaText * previewText=new ElaText(QString("预览 %1 文件").arg(asulFileName.replace(".asul",".cfg")));
        ElaText * preview2Text=new ElaText(QString("预览 %1 文件").arg(asulFileName));

        previewText->setTextPixelSize(25);
        preview2Text->setTextPixelSize(25);
        QVBoxLayout * previewLayout=new QVBoxLayout(DeployWindow);
        QPlainTextEdit * previewPlainText=new QPlainTextEdit(sumContent->getValues().join("\n").replace("\\\"","\""),DeployWindow);
        QPlainTextEdit * preview2PlainText=new QPlainTextEdit(sumAsul->getValues().join("\n"),DeployWindow);
        ...
        connect(this,&T_Deploy::changePreviewHide,[=](bool show){
        ...
        connect(deployButton,&ElaPushButton::clicked,[=](){
            ...
            connect(dialog,&ElaContentDialog::rightButtonClicked,this,[=](){
                ...
                qDebug()<<"write to "<<CFGFileLocation+"/"+AsulFileName;
                QFile *preferrenceFile=new QFile(CFGFileLocation+"/"+CFGFileName);
                // QString asulFileNa=CFGFileName;
                qDebug()<<"AsulFileName"<<AsulFileName;
                QFile *asulFi=new QFile(CFGFileLocation+"/"+AsulFileName);
                if(asulFi->exists()){
                    if (!asulFi->remove()) {
                        qWarning() << "无法删除文件:" << asulFi->fileName();
                        return;
                    }
                }
                ...
                if(!asulFi->open(QIODevice::ReadWrite)){
                    qDebug()<<"Error has occured"<< CFGFileLocation+"/"+AsulFileName;
                }
                ...
                *preferrenceStream<<"//"<<QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss");
                ...
                if(pageNumMap[CFGBreifInfo]==0){
                    ...
                    qDebug()<<"ElaPageNodeS"<<ElaPageNodeS;
